...
flowchart LR
  ...
  P -->|require_human| H[approval store]
...
| Approval | Persists a pending decision through a configured store | Provide a durable store, authenticated decision channel, and resume flow | None required for the store state machine |
...
| Process stops while held | Resume requires a durable approval store and checkpointer |
...
3. Use a durable approval store for human decisions.
...
coverage, and [SecurityContext](security-context.md) for a security-domain
application.
